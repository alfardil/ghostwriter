...
class KafkaProducerServiceTest {
  ...
  void sendMessage() {
    ...
    when(kafkaTemplate.send("task", "user1", "hello"))
  ...
  void sendMessageKafkaFailure() {
    ...
    assertThatCode(() -> kafkaProducerService.sendMessage("user1", "hello"))
  ...
  void sendReply() {
    ...
    SendResult<String, String> sendResult = mock(SendResult.class);
    when(sendResult.getRecordMetadata()).thenReturn(metadata);
    when(kafkaTemplate.send("reply", "user1", "response"))
      .thenReturn(CompletableFuture.completedFuture(sendResult));
  ...
  void sendReplyKafkaFailure() {
    ...
    assertThatCode(() -> kafkaProducerService.sendReply("user1", "response"))
      .doesNotThrowAnyException();
  }
}
